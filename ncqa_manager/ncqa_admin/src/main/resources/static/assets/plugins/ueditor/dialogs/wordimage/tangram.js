 ...
 * @cn.neusoft.xuxiao.config {Object} vars 创建imageUploader时所需要的参数
 * @cn.neusoft.xuxiao.config {Number} vars.gridWidth 每一个预览图片所占的宽度，应该为flash寛的整除
 * @cn.neusoft.xuxiao.config {Number} vars.gridHeight 每一个预览图片所占的高度，应该为flash高的整除
 * @cn.neusoft.xuxiao.config {Number} vars.picWidth 单张预览图片的宽度
 * @cn.neusoft.xuxiao.config {Number} vars.picHeight 单张预览图片的高度
 * @cn.neusoft.xuxiao.config {String} vars.uploadDataFieldName POST请求中图片数据的key,默认值'picdata'
 * @cn.neusoft.xuxiao.config {String} vars.picDescFieldName POST请求中图片描述的key,默认值'picDesc'
 * @cn.neusoft.xuxiao.config {Number} vars.maxSize 文件的最大体积,单位'MB'
 * @cn.neusoft.xuxiao.config {Number} vars.compressSize 上传前如果图片体积超过该值，会先压缩
 * @cn.neusoft.xuxiao.config {Number} vars.maxNum:32 最大上传多少个文件
 * @cn.neusoft.xuxiao.config {Number} vars.compressLength 能接受的最大边长，超过该值会等比压缩
 * @cn.neusoft.xuxiao.config {String} vars.url 上传的url地址
 * @cn.neusoft.xuxiao.config {Number} vars.mode mode == 0时，是使用滚动条，mode == 1时，拉伸flash, 默认值为0
 ...
 * @cn.neusoft.xuxiao.config {Object} createOptions 创建flash时需要的参数，请参照baidu.swf.create文档
 * @cn.neusoft.xuxiao.config {String} createOptions.width
 * @cn.neusoft.xuxiao.config {String} createOptions.height
 * @cn.neusoft.xuxiao.config {Number} maxNum 最大可选文件数
 * @cn.neusoft.xuxiao.config {Function|String} selectFile
 * @cn.neusoft.xuxiao.config {Function|String} exceedMaxSize
 * @cn.neusoft.xuxiao.config {Function|String} deleteFile
 * @cn.neusoft.xuxiao.config {Function|String} uploadStart
 * @cn.neusoft.xuxiao.config {Function|String} uploadComplete
 * @cn.neusoft.xuxiao.config {Function|String} uploadError
 * @cn.neusoft.xuxiao.config {Function|String} uploadProgress
 */
 ...
 * @cn.neusoft.xuxiao.config {String} [charset] script的字符集
 * @cn.neusoft.xuxiao.config {Integer} [timeOut] 超时时间，超过这个时间将不再响应本请求，并触发onfailure函数
 * @cn.neusoft.xuxiao.config {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
 ...
 * @cn.neusoft.xuxiao.config {string} [charset] script的字符集
 * @cn.neusoft.xuxiao.config {string} [queryField] 服务器端callback请求字段名，默认为callback
 * @cn.neusoft.xuxiao.config {Integer} [timeOut] 超时时间(单位：ms)，超过这个时间将不再响应本请求，并触发onfailure函数
 * @cn.neusoft.xuxiao.config {Function} [onfailure] timeOut设定后才生效，到达超时时间时触发本函数
 * @remark
