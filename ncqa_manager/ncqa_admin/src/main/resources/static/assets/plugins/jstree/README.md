...
Unlike the previous simple HTML example, this time the `.jstree()` function accepts a cn.neusoft.xuxiao.config object.
 ...
 * `type` - __types plugin specific__ - the type of the nodes (should be defined in the types cn.neusoft.xuxiao.config), if not set `"default"` is assumed.
...
Instead of a JS array, you can set `core.data` to a [jQuery AJAX cn.neusoft.xuxiao.config](http://api.jquery.com/jQuery.ajax/)
...
Now to focus on what is different. First off the `"data"` cn.neusoft.xuxiao.config option of the data object. If you check with jQuery, it is supposed to be a string or an object. But jstree makes it possible to set a function.
...
The above example shows how to obtain a reference to a jstree instance (again with a selector, but this time instead of a cn.neusoft.xuxiao.config, we pass a boolean `true`), and call a couple of methods - the latter one is selecting a node by its ID.
...
We already covered the cn.neusoft.xuxiao.config object in general (when we specified inline & AJAX data sources).

```js
$("#tree").jstree({ cn.cn.neusoft.xuxiao.configt.xuxiao.config });
```

Each key in the conficn.neusoft.xuxiao.configct corresponds to a plugin, and the value of that key is the configuration for that plugin. There are also two special keys `"core"` and `"plugins"`:
...
__All conficn.neusoft.xuxiao.configons and defaults are documented in the API docs__
...
jsTree comes with a few plugin bundled, but they will only modify your tree if you activate them using the `"plugins"` conficn.neusoft.xuxiao.configon. Here is a brief description of each plugin. You can read more on the available conficn.neusoft.xuxiao.configons for each plugin in the API docs.
...
$("#tree").jstree({
  "core" : {
    "data" : { .. AJAX conficn.neusoft.xuxiao.config
  },
  "massload" : {
    "url" : "/some/path",
    "data" : function (nodes) {
...
### sort
Automatically arranges all sibling nodes according to a comparison conficn.neusoft.xuxiao.configon function, which defaults to alphabetical order.
...
### More plugins
If you create your own plugin (or download a 3rd party one) you must include its source on the page and list its name in the `"plugins"` conficn.neusoft.xuxiao.configy.
...
As seen here when creating a plugin you can define a default conficn.neusoft.xuxiao.config your own functions to jstree, or override existing ones while maintaining the ability to call the overridden function.
